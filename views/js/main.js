...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    for (var i = 0; i < allRandomPizzas.length; i++) {
      allRandomPizzas[i].style.width = newSize + '%';
    }
  }
...
// This for-loop actually creates all of the pizzas when the page loads
// Once they are all created they are appended to the page
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((elementScrollTop / 1250) + (i % 5));
...
// Generates just enough sliding pizzas to cover the screen (based on
// screen height) when the page loads.
